import{j as i,h as e,g as s,f as u}from"./vendor.0091d4a3.js";import{f as D,h as R,s as S,a as G,e as N}from"./index.09481aae.js";import{s as c,S as f,a as k}from"./loading.2306138e.js";import{R as x}from"./RecommendList.f8e6787f.js";import{N as M,S as P}from"./news.2f87a230.js";const K=A=>({type:R,data:A}),y=()=>A=>{D().then(a=>{A(K(a.data.data))}).catch(a=>{console.log("\u51FA\u9519\u4E86")})};var b="/assets/defaultUserImg.e2905e9a.png";const V=c.div`
    display: flex;
    width: 100vw;
    height: 80px;
    align-items: center;
    justify-content: center;
    .userImg {
        width: 40px;
        height: 40px;
        margin: 0 20px;
        border: 2px solid #fff;
        border-radius: 50%;
        img {
            width: 40px;
            height: 40px;
        }       
    }
    .userName {
        flex: 1;
        font-size: 15px;
        font-weight: 700;
    }
    .news {
        height: 30px;
        width: 30px;
        margin: 0 20px;
        img {
            height: 30px;
            width: 30px;
        }
    } 

`,Y=()=>i(V,{children:[e("div",{className:"userImg",children:e("img",{src:b})}),e("div",{className:"userName",children:"R517tehdehj2uh"}),e("div",{className:"news",children:e("img",{src:M})})]}),U=c.div`
    display: flex;
    height: 30px;
    .userBenefits__card, .userBenefits__cash {
        line-height: 30px;
        text-align: center;
        font-size: 13px;
        flex: 1;
        font-weight: 700;
    }
    .cardsNum, .cashAmount{
        font-size: 23px;
    }
    .userBenefits__card {
        border-right: 1px solid #d3d7db;
    }
`,H=A=>i(U,{children:[i("div",{className:"userBenefits__card",children:[e("span",{className:"cardsNum",children:0}),"\u5F20\u4F18\u60E0\u5361"]}),i("div",{className:"userBenefits__cash",children:[e("span",{className:"cashAmount",children:0}),"\u5143\u73B0\u91D1\u7EA2\u5305"]})]});var J="data:image/png;base64,",v="/assets/pickUp.8ab0fef9.png",z="/assets/evaluation.dcf41585.png",q="/assets/afterSale.59ad8552.png",F="data:image/png;base64,";const L=c.div`
    height: 60px;
    background-color: #fff;
    display: flex;
    margin-top: 20px;
    border-radius: 10px;
`,T=c.div`
    flex: 1;
    display: flex;
    justify-content: center;
    align-items: center;
    .middleBox {
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
        .icon {
            width: 20px;
            height: 20px;
        }
        .desc {
            font-size: 13px;
        }
    }
    
`,Z=A=>{const{ButtomData:a=[]}=A;let g={WillPayIcon:J,PickUpIcon:v,EvaluationIcon:z,AfterSaleIcon:q,AllOrdersIcon:F};return e(L,{children:a?a.map(t=>e(T,{children:i("div",{className:"middleBox",children:[e("img",{src:g[t.icon],className:"icon"}),e("div",{className:"desc",children:t.title})]})},t.id)):e("div",{})})};const r=A=>{const{menuBarData:a=[]}=A,g=t=>{};return e("div",{className:"menu-item__box",children:a.map((t,n)=>i("div",{className:"item-box",onClick:()=>{g(`${t.type}`)},children:[e("img",{src:t.picUrl,alt:"",className:"item-box__img"}),e("div",{className:"item-box__text",children:t.type})]},n))})};const j=A=>{const{menuBarData:a={}}=A;return s.exports.useEffect(()=>{new P(".swiper",{direction:"horizontal",scrollbar:{el:".swiper-scrollbar"}})},[]),e("div",{className:"swiper-bg-my",children:e("div",{className:"swiper-region-my",children:i("div",{className:"swiper",children:[i("div",{className:"swiper-wrapper",children:[i("div",{className:"swiper-slide",children:[e(r,{menuBarData:a.list1}),e(r,{menuBarData:a.list2})]}),i("div",{className:"swiper-slide",children:[e(r,{menuBarData:a.list3}),e(r,{menuBarData:a.list4})]})]}),e("div",{className:"swiper-scrollbar"})]})})})};const O=A=>{const{myPageData:a,totalAccount:g,selectedGoods:t}=A,{menuBarData:n,OrdersOprationButtomData:I}=a;let[E,l]=s.exports.useState(1);const[o,B]=s.exports.useState([]),{getMyPageDataDispatch:d,getSelectedGoodsDisPatch:m,getTotalAccountDispatch:w}=A,h=()=>{l(++E)},Q=()=>{},C=async()=>{await N().then(p=>{B([...o,...p.data.data.list])})};return s.exports.useEffect(()=>{a.length||d()},[]),s.exports.useEffect(()=>{C()},[E]),e("div",{className:"my__container",children:e(f,{direction:"vertical",refresh:!1,pullUp:h,pullDown:Q,onScroll:()=>{k()},children:i("div",{children:[e(Y,{}),e(H,{}),i("div",{className:"padding",children:[e(Z,{ButtomData:I}),e(j,{menuBarData:n})]}),e(x,{recommendList:o,selectedGoods:t,totalAccount:g,setCartInfo:m,setTotalAccount:w})]})})})},W=A=>({myPageData:A.my.myPageData,totalAccount:A.main.totalAccount,selectedGoods:A.main.selectedGoods}),X=A=>({getMyPageDataDispatch(){A(y())},getSelectedGoodsDisPatch(a){A(S(a))},getTotalAccountDispatch(a){A(G(a))}});var ie=u(W,X)(s.exports.memo(O));export{ie as default};
