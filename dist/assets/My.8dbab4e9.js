import{j as i,h as e,g as s,f as D}from"./vendor.23ef3ccf.js";import{h as R,i as S,s as G,a as f,d as N,f as x}from"./index.1e6027d5.js";import{S as b,a as k}from"./loading.e8037d2e.js";import{R as M}from"./RecommendList.ac766406.js";import{s as c}from"./styled-components.browser.esm.ec6bb831.js";import{N as P}from"./news.ddd16e4e.js";import{S as K}from"./swiper.min.9cab8f8d.js";const y=A=>({type:S,data:A}),V=()=>A=>{R().then(a=>{A(y(a.data.data))}).catch(a=>{console.log("\u51FA\u9519\u4E86")})};var Y="/assets/defaultUserImg.e2905e9a.png";const U=c.div`
    display: flex;
    width: 100vw;
    height: 80px;
    align-items: center;
    justify-content: center;
    .userImg {
        width: 40px;
        height: 40px;
        margin: 0 20px;
        border: 2px solid #fff;
        border-radius: 50%;
        img {
            width: 40px;
            height: 40px;
        }       
    }
    .userName {
        flex: 1;
        font-size: 15px;
        font-weight: 700;
    }
    .news {
        height: 30px;
        width: 30px;
        margin: 0 20px;
        img {
            height: 30px;
            width: 30px;
        }
    } 

`,H=()=>i(U,{children:[e("div",{className:"userImg",children:e("img",{src:Y})}),e("div",{className:"userName",children:"R517tehdehj2uh"}),e("div",{className:"news",children:e("img",{src:P})})]}),J=c.div`
    display: flex;
    height: 30px;
    .userBenefits__card, .userBenefits__cash {
        line-height: 30px;
        text-align: center;
        font-size: 13px;
        flex: 1;
        font-weight: 700;
    }
    .cardsNum, .cashAmount{
        font-size: 23px;
    }
    .userBenefits__card {
        border-right: 1px solid #d3d7db;
    }
`,v=A=>i(J,{children:[i("div",{className:"userBenefits__card",children:[e("span",{className:"cardsNum",children:0}),"\u5F20\u4F18\u60E0\u5361"]}),i("div",{className:"userBenefits__cash",children:[e("span",{className:"cashAmount",children:0}),"\u5143\u73B0\u91D1\u7EA2\u5305"]})]});var z="data:image/png;base64,",q="/assets/pickUp.8ab0fef9.png",F="/assets/evaluation.dcf41585.png",T="/assets/afterSale.59ad8552.png",L="data:image/png;base64,";const j=c.div`
    height: 60px;
    background-color: #fff;
    display: flex;
    margin-top: 20px;
    border-radius: 10px;
`,Z=c.div`
    flex: 1;
    display: flex;
    justify-content: center;
    align-items: center;
    .middleBox {
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
        .icon {
            width: 20px;
            height: 20px;
        }
        .desc {
            font-size: 13px;
        }
    }
    
`,O=A=>{const{ButtomData:a=[]}=A;let g={WillPayIcon:z,PickUpIcon:q,EvaluationIcon:F,AfterSaleIcon:T,AllOrdersIcon:L};return e(j,{children:a?a.map(t=>e(Z,{children:i("div",{className:"middleBox",children:[e("img",{src:g[t.icon],className:"icon"}),e("div",{className:"desc",children:t.title})]})},t.id)):e("div",{})})};const r=A=>{const{menuBarData:a=[]}=A,g=t=>{};return e("div",{className:"menu-item__box",children:a.map((t,n)=>i("div",{className:"item-box",onClick:()=>{g(`${t.type}`)},children:[e("img",{src:t.picUrl,alt:"",className:"item-box__img"}),e("div",{className:"item-box__text",children:t.type})]},n))})};const W=A=>{const{menuBarData:a={}}=A;return s.exports.useEffect(()=>{new K(".swiper",{direction:"horizontal",scrollbar:{el:".swiper-scrollbar"}})},[]),e("div",{className:"swiper-bg-my",children:e("div",{className:"swiper-region-my",children:i("div",{className:"swiper",children:[i("div",{className:"swiper-wrapper",children:[i("div",{className:"swiper-slide",children:[e(r,{menuBarData:a.list1}),e(r,{menuBarData:a.list2})]}),i("div",{className:"swiper-slide",children:[e(r,{menuBarData:a.list3}),e(r,{menuBarData:a.list4})]})]}),e("div",{className:"swiper-scrollbar"})]})})})};const X=A=>{const{myPageData:a,totalAccount:g,selectedGoods:t}=A,{menuBarData:n,OrdersOprationButtomData:I}=a;let[o,l]=s.exports.useState(1);const[E,d]=s.exports.useState([]),{getMyPageDataDispatch:B,getSelectedGoodsDisPatch:m,getTotalAccountDispatch:w,getTabbarIndexDiapatch:p}=A,h=()=>{l(++o)},Q=()=>{},C=async()=>{await x(o).then(u=>{d([...E,...u.data.data.list])})};return s.exports.useEffect(()=>{p(3),a.length||B()},[]),s.exports.useEffect(()=>{C()},[o]),e("div",{className:"my__container",children:e(b,{direction:"vertical",refresh:!1,pullUp:h,pullDown:Q,onScroll:()=>{k()},children:i("div",{children:[e(H,{}),e(v,{}),i("div",{className:"padding",children:[e(O,{ButtomData:I}),e(W,{menuBarData:n})]}),e(M,{recommendList:E,selectedGoods:t,totalAccount:g,setCartInfo:m,setTotalAccount:w})]})})})},_=A=>({myPageData:A.my.myPageData,totalAccount:A.main.totalAccount,selectedGoods:A.main.selectedGoods}),$=A=>({getMyPageDataDispatch(){A(V())},getSelectedGoodsDisPatch(a){A(G(a))},getTotalAccountDispatch(a){A(f(a))},getTabbarIndexDiapatch(a){A(N(a))}});var ce=D(_,$)(s.exports.memo(X));export{ce as default};
