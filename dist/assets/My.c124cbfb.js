import{j as a,f as e,h as t,g as D,u as R}from"./vendor.b5b01676.js";import{e as f,k as S}from"./index.bde435c7.js";import{g as N}from"./actionCreators.87fc8412.js";import{a as x,f as b}from"./add.f991108b.js";import{R as G}from"./RecommendList.afbe9f13.js";import{s as c}from"./styled-components.browser.esm.e5d3f427.js";import{N as k}from"./news.ddd16e4e.js";import{S as M}from"./swiper.min.43eced01.js";var K="/assets/defaultUserImg.e2905e9a.png";const P=c.div`
    display: flex;
    width: 100vw;
    height: 80px;
    align-items: center;
    justify-content: center;
    .userImg {
        width: 40px;
        height: 40px;
        margin: 0 20px;
        border: 2px solid #fff;
        border-radius: 50%;
        img {
            width: 40px;
            height: 40px;
        }       
    }
    .userName {
        flex: 1;
        font-size: 15px;
        font-weight: 700;
    }
    .news {
        height: 30px;
        width: 30px;
        margin: 0 20px;
        img {
            height: 30px;
            width: 30px;
        }
    } 

`,y=A=>a(P,{children:[e("div",{className:"userImg",children:e("img",{src:K})}),e("div",{className:"userName",children:"\u70B9\u51FB\u767B\u5F55"}),e("div",{className:"news",children:e("img",{src:k})})]}),V=c.div`
    display: flex;
    height: 30px;
    .userBenefits__card, .userBenefits__cash {
        line-height: 30px;
        text-align: center;
        font-size: 13px;
        flex: 1;
        font-weight: 700;
    }
    .cardsNum, .cashAmount{
        font-size: 23px;
    }
    .userBenefits__card {
        border-right: 1px solid #d3d7db;
    }
`,Y=A=>a(V,{children:[a("div",{className:"userBenefits__card",children:[e("span",{className:"cardsNum",children:0}),"\u5F20\u4F18\u60E0\u5361"]}),a("div",{className:"userBenefits__cash",children:[e("span",{className:"cashAmount",children:0}),"\u5143\u73B0\u91D1\u7EA2\u5305"]})]});var U="data:image/png;base64,",H="/assets/pickUp.8ab0fef9.png",J="/assets/evaluation.dcf41585.png",v="/assets/afterSale.59ad8552.png",z="data:image/png;base64,";const F=c.div`
    height: 60px;
    background-color: #fff;
    display: flex;
    margin-top: 20px;
    border-radius: 10px;
`,q=c.div`
    flex: 1;
    display: flex;
    justify-content: center;
    align-items: center;
    .middleBox {
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
        .icon {
            width: 20px;
            height: 20px;
        }
        .desc {
            font-size: 13px;
        }
    }
    
`,L=A=>{const{ButtomData:i=[]}=A;let g={WillPayIcon:U,PickUpIcon:H,EvaluationIcon:J,AfterSaleIcon:v,AllOrdersIcon:z};return e(F,{children:i?i.map(s=>e(q,{children:a("div",{className:"middleBox",children:[e("img",{src:g[s.icon],className:"icon"}),e("div",{className:"desc",children:s.title})]})},s.id)):e("div",{})})};const n=A=>{const{menuBarData:i=[]}=A,g=s=>{};return e("div",{className:"menu-item__box",children:i.map((s,r)=>a("div",{className:"item-box",onClick:()=>{g(`${s.type}`)},children:[e("img",{src:s.picUrl,alt:"",className:"item-box__img"}),e("div",{className:"item-box__text",children:s.type})]},r))})};const j=A=>{const{menuBarData:i={}}=A;return t.exports.useEffect(()=>{new M(".swiper",{direction:"horizontal",scrollbar:{el:".swiper-scrollbar"}})},[]),e("div",{className:"swiper-bg-my",children:e("div",{className:"swiper-region-my",children:a("div",{className:"swiper",children:[a("div",{className:"swiper-wrapper",children:[a("div",{className:"swiper-slide",children:[e(n,{menuBarData:i.list1}),e(n,{menuBarData:i.list2})]}),a("div",{className:"swiper-slide",children:[e(n,{menuBarData:i.list3}),e(n,{menuBarData:i.list4})]})]}),e("div",{className:"swiper-scrollbar"})]})})})};const Z=A=>{const{myPageData:i}=A,{menuBarData:g,OrdersOprationButtomData:s}=i;let[r,I]=t.exports.useState(1);const[E,l]=t.exports.useState([]),[B,o]=t.exports.useState(!1),m=R(),{getMyPageDataDispatch:d,getTabbarIndexDiapatch:w}=A,p=async()=>{B||await setTimeout(()=>{o(!0),I(++r)},1e3),o(!1)},Q=()=>{},C=()=>{m.push("/login")},h=async()=>{await S(r).then(u=>{l([...E,...u.data.data.list])})};return t.exports.useEffect(()=>{w(3),i.length||d()},[]),t.exports.useEffect(()=>{h()},[r]),e("div",{className:"my__container",children:e(x,{direction:"vertical",refresh:!1,pullUp:p,pullDown:Q,onScroll:()=>{b()},children:a("div",{children:[e(y,{UserLogin:C}),e(Y,{}),a("div",{className:"padding",children:[e(L,{ButtomData:s}),e(j,{menuBarData:g})]}),e(G,{recommendList:E})]})})})},T=A=>({myPageData:A.my.myPageData}),O=A=>({getMyPageDataDispatch(){A(N())},getTabbarIndexDiapatch(i){A(f(i))}});var se=D(T,O)(t.exports.memo(Z));export{se as default};
